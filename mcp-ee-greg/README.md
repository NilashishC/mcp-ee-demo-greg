...
- `mcp-ee-greg.yaml` - This is the main definition file that `ansible-builder` will use to construct your EE image.
...
cd mcp-ee-demo-greg/mcp-ee-greg
```

Open `mcp-ee-greg.yaml` in your favorite text editor and review the configuration.
...
Before you build the EE, you must ensure `ansible-builder` can access all the collections you listed in your `mcp-ee-greg.yaml` file.
...
```bash
ansible-builder build --file mcp-ee-greg.yaml --tag mcp-ee-greg:latest --container-runtime podman
```

This command uses your `mcp-ee-greg.yaml` file to build an image and tags it as `mcp-ee-greg:latest`
...
```bash
ansible-navigator run playbook.yml --execution-environment-image mcp-ee-greg:latest
```
...
1. Tag your local image for the target registry (e.g., `quay.io/your-username/mcp-ee-greg:latest`).

```bash
podman tag mcp-ee-greg:latest quay.io/your-username/mcp-ee-greg:latest
```

2. Push the tagged image to the target registry:

```bash
podman push quay.io/your-username/mcp-ee-greg:latest
```
